   ...
   */
    ...
    */
    ...
    fulScr() {
        ...
        ['ff', 'n'].forEach(v => _c$1(w)[v]("fullscreenchange", this.isfulScr.bind(this)));
        return this;
    }
    isfulScr() {
    ...
    //* move to one of the media modules/plugins i dont think it has use on a normal webpages.
    stop(el) {
        var a = typeof el == 'object' ? el : _c$1(el).src();
        a.getTracks().forEach(trk => trk.stop());
        if (typeof el == 'string') _c$1(el).src('');
    }
    ...
    static mkMObs(cb, el, opt = null) {
        const { utils: { type: t } } = __o;
        const options = {
            childList: false,       // Tracks changes to child elements of the target node.
            attributes: false,      // Tracks changes to attributes of the target node.
            characterData: false,   // Tracks changes to text content in the target node.
            subtree: false,         // Tracks changes to all descendants of the target node.
            attributeOldValue: false, // Records previous value of the attributes (only if `attributes` is true).
            characterDataOldValue: false, // Records previous value of text content (only if `characterData` is true).
            attributeFilter: null,  // Filters observed attributes (only if `attributes` is true).
        };
        if (t(opt, 'obj'))
            this.each(opt, 'e', ([k, v]) => (options[k] = t(v, 'func') ? v() : v));
        let obs = new MutationObserver(cb);
        obs.opts = options;
        t(el, 'inst', HTMLElement) ? obs.observe(el, options) : 0;
        return obs;
    }
    /*lite_end*/
    //-------
...
/* =================== the End =========================== */window.__o=__o$1;window._c=_c$1;window._n=_n;window._o=_o;return exports;})({});